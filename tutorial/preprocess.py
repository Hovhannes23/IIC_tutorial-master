...
def create_MNIST_np_files_with_preprocessed_augs(
    alb_transforms,
    target_dir=".",
    aug_number=10,
    fraction_to_take=1.0,
    batch_size=4,
    num_workers=1,
):
    ...
    dataset_full = AlbMNIST(
        root=os.path.join(target_dir, "MNIST"), download=True, alb_transforms=None
    )

    if fraction_to_take < 1.0:
    ...
    save_dataset_as_numpy(
        ...
        original_mnist_path,
        key="original",
        message="Saving original dataset as np file",
    )
    save_dataset_as_numpy(
        dataloader, labels_path, key="label", message="Saving labels as np file"
    )
    ...
    for i in range(aug_number):
        ...
        save_dataset_as_numpy(
            ...
            aug_mnist_path,
            key="augmented",
            message="Creating augmented dataset #%i" % i,
        )
...
def create_MNIST_arrays(
    ...
    dataset_alb = AlbMNIST(os.path.join(target_dir), download=True)
    dataloader = DataLoader(
...
def manually_download_MNIST(DATASET_DIR):
    ...
    # url = "https://github.com/Hovhannes23/Cities/releases/download/publish/MNIST.zip"
